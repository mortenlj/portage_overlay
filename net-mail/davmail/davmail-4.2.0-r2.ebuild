...
# Tests require access to ports and files not available as ebuild?
RESTRICT="test"
...
EANT_GENTOO_CLASSPATH="commons-codec,commons-httpclient-3,htmlcleaner,jcifs-1.1,log4j,swt-3.7,stax2-api,javamail,servlet-api-2.5,wstx-4.1"
...
src_prepare() {
	...
	find src/ -name "OSX*" -and -not -name "OSXInfoPlist.java" -delete || die
	sed -i "/OSX[A-Z]/d" src/java/davmail/ui/{browser/DesktopBrowser,tray/DavGatewayTray}.java || die
	find src/ -name "DavService.java" -delete || die
}
...
src_test() {
	local DIR="src/test"
	local CP="${DIR}:${PN}.jar:lib/jackrabbit-webdav-2.4.3.jar:$(java-pkg_getjars junit-4,${EANT_GENTOO_CLASSPATH})"
