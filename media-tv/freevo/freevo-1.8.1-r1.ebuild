...
IUSE="directfb doc dvd dvdbackup fbcon gphoto2 ivtv lirc matrox mixer mplayer nls sqlite tvtime vlc X xine xmltv"
...
RDEPEND=">=dev-python/pygame-1.5.6
	...
	xine? ( >=media-video/xine-ui-0.9.22 )
	vlc? ( media-video/vlc )
...
pkg_setup() {
	if use directfb ; then
		use dvd && use xine && ! built_with_use media-libs/xine-lib directfb \
	...
	if use fbcon ; then
		use dvd && use xine && ! built_with_use media-libs/xine-lib fbcon \
	...
	if ! { use X || use directfb || use fbcon || use matrox ; } ; then
        ...
        if ! { use mplayer || use xine || use vlc ; } ; then
                eerror "ERROR - no audio/video player selected."
                eerror "Please activate the use flags to activate either of these players:"
                eerror " mplayer xine vlc "
		die "Re-emerge freevo with multimedia player support. (USE=\"mplayer xine vlc\")"
        fi
...
pkg_postinst() {
	...
	if use X ; then
		...
		echo
		ewarn "Dvd-menu navigation varies with each multimedia player."
		ewarn "Please check the manual pages of your preferred player (mplayer, xine or vcl)."
